...
function setupRestClient(host: string, session?: SessionType): KarrioClient {
  ...
  const modifiedConfig: InternalAxiosRequestConfig<any> = requestInterceptor(session)(config);
...
function setupGraphQLClient(host: string, session?: SessionType) {
  ...
  axiosInstance.interceptors.request.use((config: AxiosRequestConfig) => Promise.resolve(requestInterceptor(session)(config)));
